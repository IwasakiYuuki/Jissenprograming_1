...
int read_data(void)
{
	...
	int i;
	/* キャラクタデータ読み込み */
	fp = fopen("station_data.txt","r"); //の読み込み
		if ( fp == NULL ){
...
STATION *check(char target[])
{
	...
	current_addr = head.next_addr;
	while(1){
		if( !strcmp(current_addr->name,target) )
			return current_addr;
		if(current_addr->next_addr == NULL)
			break;
		/* current_addrを更新 */
...
void add(void)
{
	...
	do{
		...
		targetChar[i] = '\0';
		/* ↑改行コードを削除するための処理 */
	} while( (addr=check(targetChar)) == ERROR );
	/* データの追加（リストをつなぐ） */
	station[a_point].next_addr=addr->next_addr;
	addr->next_addr=&station[a_point];
	station_disp();
}
...
void del(void)
{
	...
	current_addr = head.next_addr;
	/* 残り１つだったらそれ以上消さない */
	if(current_addr->next_addr==NULL)return;
	while(1){
