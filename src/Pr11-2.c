...
int main(void)
{
    ...
    flag=0;
	do{
	if(i!=0)printf("都市名が存在しません. もう一度出発都市を入力して下さい.　：");
	else printf("出発都市を入力して下さい.　：");
	scanf("%s",start);
	
	for(i=0;i<MAX_SIZE;i++){
		if(strcmp(start,graph_data[i])==0){
			flag=1;	
			begin=i;
		}
	}
	...
	flag=0;
	do{
	if(i!=0)printf("都市名が存在しません. もう一度到着都市を入力して下さい.　：");
	else printf("到着都市を入力して下さい.　：");
	scanf("%s",destination);
	
	for(i=0;i<MAX_SIZE;i++){
		if(strcmp(destination,graph_data[i])==0){
			flag=1;
			end=i;
		}	
	}
